...
ProjectionFactor::ProjectionFactor(const double x, const double y, const double shift) : x(x), y(y), shift(shift)
{
    ;
}
...
//    par_pose[6]= cur_q.w();
    ...
    Eigen::Vector3d f_w(parameters[1][0], parameters[1][1], parameters[1][2]);
...
//    if ( std::isnan(parameters[0][3]) ||
//         std::isnan(parameters[0][4]) ||
//         std::isnan(parameters[0][5]) ||
//         std::isnan(parameters[0][6]) ||
//         std::isinf(parameters[0][3]) ||
//         std::isinf(parameters[0][4]) ||
//         std::isinf(parameters[0][5]) ||
//         std::isinf(parameters[0][6])
//         ){
//        std::cout << "f_w " << f_w.transpose() << "\n" ;
//        std::cout << "f_i " << f_i.transpose() << "\n" ;
...
//    }
    ...
    if (jacobians)
    {
        if (jacobians[0])
        {
            ...
            if ( invalid ){
                jacobian_pose_j.setConstant(1.0);
            }
            else
            {
