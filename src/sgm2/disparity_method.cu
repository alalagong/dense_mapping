...
void SGM2::init_disparity_method(const uint8_t _p1, const uint8_t _p2)
{
...
void SGM2::finish_disparity_method()
{
    if(!first_alloc){
        free_memory();
...
cv::Mat SGM2::compute_disparity_method(cv::Mat left, cv::Mat right, float *elapsed_time_ms )
{
